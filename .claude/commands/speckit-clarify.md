...
2. Load the current spec file. Perform a structured ambiguity & coverage scan using this taxonomy. For each category, mark status: Clear / Partial / Missing. Produce an internal coverage map used for prioritization (do not output raw map unless no questions will be asked).
   ...
   **2a. Self-Resolution Pre-Pass (Autonomous Attempt)**:

   Before generating clarification questions for the user, attempt to self-resolve ambiguities:

   1. For each ambiguity identified in the scan above:
      a. Search `spec-kit/ai/knowledge/` documents for answers (architecture.md, data-model-spec.md, agentic-spec.md, api-spec.md, integration-spec.md, security-spec.md, frontend-spec.md, deployment-spec.md, POD.md, nfr-spec.md, glossary.yaml)
      b. Search the workspace codebase for existing patterns or implementations that answer the question
      c. If a JIRA issue ID is present in spec.md header, run `python spec-kit/.specify/scripts/python/jira_ops.py fetch <ISSUE_ID>` and check for relevant context
   2. Classify each ambiguity as:
      - **Resolved**: Answer found -- update spec.md in working memory with `[Resolved from {source}]` annotation
      - **Assumed**: Reasonable default available -- note assumption with `[Assumed: {reason}]`
      - **Open**: Genuinely requires user input -- keep for question generation
   3. Only generate clarification questions for **Open** ambiguities
   4. Hard cap: maximum 5 questions after self-resolution
   5. If JIRA_PAT not configured or API calls fail: skip JIRA check silently (graceful degradation)
...
5. Integration after EACH accepted answer (incremental update approach):
    ...
    - Then immediately apply the clarification to the most appropriate section(s):
       ...
       - Non-functional constraint â†’ Add/modify measurable criteria in Success Criteria > Measurable Outcomes (convert vague adjective to metric or explicit target).
